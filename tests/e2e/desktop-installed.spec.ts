...
test("已安装桌面 EXE 可运行 Shell 终端并发送 Ctrl+C", async () => {
  ...
  try {
    ...
    await window.getByLabel("终端输入").fill("echo installed-terminal-e2e");
    await window.getByRole("button", { name: "发送输入" }).click();
    await expect(window.getByLabel("终端输出", { exact: true })).toContainText("installed-terminal-e2e");
    await window.getByRole("button", { name: "发送 Ctrl+C" }).click();
