...
本仓库完成的是本地桌面 AI 工程工作流平台的 MVP 基础实现与验证路径。它已经覆盖从协议识别、Harness 导入、canonical workflow 存储、event-sourced run projection、kernel transition、治理边界、执行边界、renderer workbench 到恢复投影重建的核心闭环。

本审计不声明最终完整产品已经完成。高级运行时、真实外部 provider、完整桌面进程管理、完整 API 和知识发布等能力仍为后续工作。
...
- `npm.cmd run verify`：通过。contracts 4 个测试通过，renderer 4 个测试通过，desktop 测试/类型检查通过，runtime pytest 95 个测试通过；pytest 输出 1 个 `StarletteDeprecationWarning`，不影响通过结果。默认 verify 不运行浏览器 E2E。
- `npm.cmd run test:e2e`：通过。Playwright Chromium 1 个 smoke 测试通过，验证 MVP workbench 中 `Projects`、`Run Dashboard`、`Recovery` 可见。
- `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1`：通过。执行与默认 verify 等价的 PowerShell 验证路径，runtime pytest 95 个测试通过，并保留同一个 `StarletteDeprecationWarning`。
...
| Kernel transition | `runtime/tests/test_kernel.py` 覆盖 node start、artifact submit、approval、gate、revision conflict、非法 transition、自动完成等路径。 | `runtime/src/workflow_platform/kernel/transition.py` |
| Approval-Gate-Artifact governance | `runtime/tests/test_governance.py` 覆盖 artifact path sandbox、artifact hash、gate actor/evidence 约束；kernel 测试覆盖 human approval 权限。 | `runtime/src/workflow_platform/artifacts/service.py`, `runtime/src/workflow_platform/approvals/service.py`, `runtime/src/workflow_platform/gates/service.py` |
...
| Runtime API health | `runtime/tests/test_api.py` 验证 FastAPI `/health` endpoint。 | `runtime/src/workflow_platform/api/app.py`, `runtime/src/workflow_platform/main.py` |
| Electron desktop boundary | `apps/desktop/test/main.test.ts`、`apps/desktop/test/runtime-health.test-d.ts` 覆盖 desktop main/preload/runtime 的基础边界与类型。 | `apps/desktop/src/main/*.ts`, `apps/desktop/src/preload/*.ts` |
...
- Runtime API 目前以 health 和测试覆盖的基础路径为主，不是完整产品 API。
- `run_events` 与 `run_projections` 的 schema、projection 和 workflow version repository 已验证，但真实持久化 run repository、事务化 append event、并发恢复服务仍需后续实现。
- Electron main/preload 已建立基础边界，但完整 runtime process management、端口发现、崩溃重启、日志采集和打包发布仍未完成。
- Renderer workbench 是 MVP 信息架构和 smoke path，不是完整交互式产品 UI。
